...
function Hrana() {
	...
	const fetchData = useCallback(async () => {
		try {
			setLoader(true);
			const res = await fetch(`/api/hrana/${router.query.id}`);
	...
	async function onSave(formData) {
		try {
			...
			const res = await fetch(`/api/hrana/${data.id}`, {
				method: "PATCH",
				headers: {
			...
			setTimeout(() => {
				...
				createNotification({
					...
					message: `Hrana uspešno ažurirana`
				});
				router.push("/hrana");
			}, 1 * 1000);
		} catch (error) {
	...
	return (
		<div>
			<Head>
				<title>{data?.naziv ?? "Hrana"} | The Reptile House</title>
				<meta
					name="description"
					content="Stranica jedne hrane"
				/>
			</Head>
			<PageTitle>{data?.naziv || "Učitavanje"}</PageTitle>
			<ProductForm kategorija="hrana" proizvod={data} onSave={onSave} />
		</div>
	)
}
