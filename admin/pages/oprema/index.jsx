...
function Oprema() {
	...
	return (
		<div>
			<Head>
				<title>Oprema | The Reptile House</title>
				<meta
					name="description"
					content="Stranica sa svom opremom"
				/>
			</Head>
			...
			>
				...
				>
					<div style={{display: "inline", width: "auto"}}>
						...
						>
							<Link href={"/oprema/kreiraj"}><AiOutlinePlus color="white" size={40} /></Link>
						</GradientButton>
					</div>
				</Tooltip>
			</div>
			{ data && (
				data.length > 0 ?	
				<ProductList data={data} category="oprema" refresh={fetchData} /> :
				<PageTitle level={6} style={{fontSize: 24, marginTop: "5rem"}}>Nema opreme za prikaz</PageTitle>
			)}
		</div>
	)
}
