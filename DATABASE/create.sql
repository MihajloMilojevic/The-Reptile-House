...
CREATE VIEW porudzbine_proizvodi AS ( SELECT CASE k.naziv WHEN 'terarijumi' THEN JSON_SET( (json_terarijum(pr.id)), '$.cena', pp.cena, '$.kolicina', pp.kolicina, '$.boja', pp.boja, '$.natpis', pp.natpis ) WHEN 'zivotinje' THEN JSON_SET( (json_zivotinja(pr.id)), '$.cena', pp.cena, '$.kolicina', pp.kolicina ) WHEN 'hrana' THEN JSON_SET( (json_hrana(pr.id)), '$.cena', pp.cena, '$.kolicina', pp.kolicina ) ELSE JSON_SET( (json_oprema(pr.id)), '$.cena', pp.cena, '$.kolicina', pp.kolicina ) END as json, pp.porudzbina_id FROM proizvodi_porudzbine pp JOIN proizvodi pr ON pr.id = pp.proizvod_id JOIN kategorije k ON k.id = pr.kategorija_id);